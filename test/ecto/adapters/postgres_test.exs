...
defmodule Ecto.Adapters.PostgresTest do
  ...
  test "alter table modify to identity" do
    alter =
      {:alter, table(:posts),
       [
    ...
    assert execute_ddl(alter) == [
             """
             ALTER TABLE "posts"
             ALTER COLUMN "field" TYPE bigint,
             ALTER COLUMN "field" ADD GENERATED BY DEFAULT AS IDENTITY
             """
             |> remove_newlines
           ]
  end

  test "alter table modify to identity with options" do
    alter =
      {:alter, table(:posts),
       [
         {:modify, :field, :identity, [start_value: 1_000, increment: 10]}
       ]}

    assert execute_ddl(alter) == [
             ~s|ALTER TABLE "posts" ALTER COLUMN "field" TYPE bigint, ALTER COLUMN "field" ADD GENERATED BY DEFAULT AS IDENTITY(START WITH 1000 INCREMENT BY 10) |
           ]
  end
