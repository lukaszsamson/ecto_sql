if Code.ensure_loaded?(Postgrex) do
  defmodule Ecto.Adapters.Postgres.Connection do
    ...
    # `:identity`'s "GENERATED BY DEFAULT AS IDENTITY" is valid as part of a column
    # definition (CREATE TABLE/ADD COLUMN), but PostgreSQL rejects it combined with
    # ALTER COLUMN ... TYPE - it must be added via a separate ALTER COLUMN ... ADD
    # GENERATED clause (see modify_identity/3 below).
    defp modify_column_type(:identity, opts), do: column_type_name(:identity, opts)
    defp modify_column_type(type, opts), do: column_type(type, opts)

    defp modify_identity(name, :identity, opts) do
      [", ALTER COLUMN ", quote_name(name), " ADD", identity_generated_expr(opts)]
    end

    defp modify_identity(_name, _type, _opts), do: []
    ...
    defp identity_generated_expr(opts) do
      ...
      case sequence do
        ...
        _ -> [" GENERATED BY DEFAULT AS IDENTITY(", Enum.join(sequence, " "), ") "]
      end
    end
